...
object DwdETHTransaction {
  ...
  def main(args: Array[String]): Unit = {
    ...
    readOdsDataBase = spark.sparkContext.getConf.get("spark.dwdETHTransaction.readOdsDataBase")
    readBlockTableName = spark.sparkContext.getConf.get("spark.dwdETHTransaction.readBlockTableName")
    readBaseTableName = spark.sparkContext.getConf.get("spark.dwdETHTransaction.readBaseTableName")
    readTraceTableName = spark.sparkContext.getConf.get("spark.dwdETHTransaction.readTraceTableName")
    readCallsTableName = spark.sparkContext.getConf.get("spark.dwdETHTransaction.readCallsTableName")
    readUnclesTableName = spark.sparkContext.getConf.get("spark.dwdETHTransaction.readUnclesTableName")
    readDmDataBase = spark.sparkContext.getConf.get("spark.dwdETHTransaction.readDmDataBase")
    readPriceTableName = spark.sparkContext.getConf.get("spark.dwdETHTransaction.readPriceTableName")
    ...
    transaction_date = spark.sparkContext.getConf.get("spark.dwdETHTransaction.transaction_date")
  ...
  def getDwdETHTransaction(spark: SparkSession): Unit = {
    ...
    /**
      * 计算手续费
      */
    spark.read.table(s"${readOdsDataBase}.${readBaseTableName}").where(s" transaction_date = '${transaction_date}' ")
    ...
    val targetDF = spark.sql(
      s"""
        ...
        |select
         ...
         |from  target t1
         |left join
         |    (select price_us, transaction_date from ${readDmDataBase}.${readPriceTableName} where id = 'ethereum' and transaction_date = '${transaction_date}' ) t2
         |on
