...
/**
  * token transaction info by lucas 20181115
  */
object DwdETHTokenTransaction {
  ...
  readDmTokenAddressTableName, readDmTokenPriceTableName, writeDataBase, writeTableName, dateTime: String = _
  ...
  def main(args: Array[String]): Unit = {
    ...
    dateTime = spark.sparkContext.getConf.get("spark.dwdETHTokenTransaction.transactionDate")
  ...
  def getDwdETHTokenTransaction(spark: SparkSession): Unit = {
    ...
    //val query_sql = if (dateTime != "") s" transaction_date = '${dateTime}' " else " 1 = 1 "
    ...
    spark.read.table(s"${readOdsDatabase}.${readLogsTableName}")
      .where(" logs_address != '' and logs_topics_one = '0xddf252ad1be2c89b69c2b068fc378daa952ba7f163c4a11628f55a4df523b3ef' " +
