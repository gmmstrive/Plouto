...
object DwETHTokenVolumeDh {
  ...
  def main(args: Array[String]): Unit = {
    ...
    readDatabase = spark.sparkContext.getConf.get("spark.dwETHTokenVolumeDh.readDatabase")
    readTableName = spark.sparkContext.getConf.get("spark.dwETHTokenVolumeDh.readTableName")
    readDmDatabase = spark.sparkContext.getConf.get("spark.dwETHTokenVolumeDh.readDmDatabase")
    readDmTableName = spark.sparkContext.getConf.get("spark.dwETHTokenVolumeDh.readDmTableName")
    writeDataBase = spark.sparkContext.getConf.get("spark.dwETHTokenVolumeDh.writeDataBase")
    writeTableName = spark.sparkContext.getConf.get("spark.dwETHTokenVolumeDh.writeTableName")
    transactionDate = spark.sparkContext.getConf.get("spark.dwETHTokenVolumeDh.transactionDate")

    getDwETHTokenVolumeDh(spark)

    spark.stop()

  }

  def getDwETHTokenVolumeDh(spark: SparkSession): Unit = {
    ...
    val tempDF = spark.sql(
      s"""
         ...
         |left join(
         |    select
         |        sum(cast(value as double)) as value_num,token_address,dh,transaction_date
